 ...
 * @title VMGToken
 * @author VMGT Project
 * @notice Production ERC-20 token with burnable, mintable, pausable, capped supply, time-locked transfers, and transfer tax.
 ...
 */
contract VMGToken is ERC20, ERC20Burnable, ERC20Pausable, ERC20Capped, Ownable {
    ...
    function lockTokens(
        ...
        require(
            ...
            "VMGToken: lock amount exceeds balance"
        );
        require(
            unlockAt > block.timestamp,
            "VMGToken: unlock time must be in the future"
        );
     ...
     */
    function _effectiveLockedBalance(
        address account
    ...
    function _update(
        ...
        if (from == address(0)) {
            ...
            if (totalSupply() > cap())
                revert ERC20ExceededCap(totalSupply(), cap());
            return;
        }
        // Burn operation
        if (to == address(0)) {
        ...
        // Transfer operation - enforce pause, lock, and tax
        _requireNotPaused();
        ...
        if (locked > 0) {
            ...
            require(
                value <= spendable,
                "VMGToken: transfer exceeds spendable (locked)"
            );
        }
        if (
            _taxEnabled &&
