...
describe("VMGToken_v2", function () {
  ...
  it("deploys with correct name and symbol", async function () {
    ...
    expect(await token.name()).to.equal("VMGToken_v2");
    expect(await token.symbol()).to.equal("VMGT2");
  });
  ...
  it("cannot mint beyond cap", async function () {
    ...
    await expect(token.mint(owner.address, extra + 1n)).to.be.revertedWithCustomError(
      token,
      "ERC20ExceededCap",
    );
  });
  ...
  it("mint fails when cap would be exceeded", async function () {
    ...
    await expect(token.mint(account1.address, overCap)).to.be.revertedWithCustomError(
      token,
      "ERC20ExceededCap",
    );
  });
  ...
  it("getRemainingMintable returns correct value", async function () {
    const { token, owner } = await deployToken();
    expect(await token.remainingMintable()).to.equal(SUPPLY_CAP - INITIAL_SUPPLY);
    await token.mint(owner.address, 100_000n * 10n ** 18n);
    expect(await token.remainingMintable()).to.equal(SUPPLY_CAP - INITIAL_SUPPLY - 100_000n * 10n ** 18n);
  });
  ...
  it("lockTokens reverts when lock amount exceeds balance", async function () {
    ...
    const block = await ethers.provider.getBlock("latest");
    await expect(
      token.lockTokens(account1.address, 2_000n * 10n ** 18n, BigInt(block.timestamp) + 3600n),
  ...
  it("non-owner cannot lock tokens", async function () {
    ...
    const block = await ethers.provider.getBlock("latest");
    await expect(
      token.connect(account1).lockTokens(account2.address, 5_000n * 10n ** 18n, BigInt(block.timestamp) + 3600n),
    ).to.be.revertedWithCustomError(token, "OwnableUnauthorizedAccount");
  });
  ...
  it("tax rate cannot exceed maximum (5%)", async function () {
    const { token } = await deployToken();
    await expect(token.setTaxRate(501n)).to.be.revertedWith("VMGToken: tax rate max 5%");
  ...
  it("minting does not trigger tax", async function () {
    ...
    expect(await token.balanceOf(account1.address)).to.equal(50_000n * 10n ** 18n);
    expect(await token.balanceOf(account1.address)).to.equal(before + 50_000n * 10n ** 18n);
  });
  ...
  it("non-owner cannot set tax rate", async function () {
    const { token, account1 } = await deployToken();
    await expect(token.connect(account1).setTaxRate(100n)).to.be.revertedWithCustomError(
      token,
      "OwnableUnauthorizedAccount",
    );
  });
  ...
  it("all features work together: cap, lock, tax, pause", async function () {
    ...
    const block = await ethers.provider.getBlock("latest");
    await token.lockTokens(account1.address, 50_000n * 10n ** 18n, BigInt(block.timestamp) + 3600n);
    await token.connect(account1).transfer(account2.address, 50_000n * 10n ** 18n);
    expect(await token.balanceOf(account2.address)).to.equal(50_000n * 10n ** 18n);
    ...
    expect(await token.balanceOf(account2.address)).to.equal(51_000n * 10n ** 18n);
  });
});
